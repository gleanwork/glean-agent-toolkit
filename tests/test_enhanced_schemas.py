...
try:
    from glean.agent_toolkit.adapters.openai import HAS_OPENAI, OpenAIAdapter
except ImportError:
    HAS_OPENAI = False
...
class TestFieldInfoExtraction:
    ...
    def test_extract_annotated_with_examples(self):
        ...
        annotation = Annotated[
            str,
            Field(description="Query with examples", examples=["example1", "example2"]),
        ]
...
class TestEnhancedToolSpecs:
    ...
    def test_enhanced_tool_schema_generation(self):
        ...
        def test_function(
            query: Annotated[
                str, Field(description="Test query parameter", examples=["test example"])
            ],
        ) -> dict[str, Any]:
            return {}
    ...
    def test_enhanced_tool_openai_adapter(self):
        ...
        def test_function(
            query: Annotated[
                str, Field(description="Enhanced query parameter", examples=["enhanced example"])
            ],
        ) -> dict[str, Any]:
            return {}
    ...
    def test_mixed_parameters(self):
        ...
        def test_function(
            enhanced_param: Annotated[
                str, Field(description="Enhanced parameter", examples=["example"])
            ],
            basic_param: str,
        ) -> dict[str, Any]:
            return {}
