...
from glean.toolkit.cli import app
...
def test_list_tools(runner: CliRunner, mock_registry: mock.MagicMock) -> None:
    """Test listing tools."""
    with mock.patch("glean.toolkit.cli.get_registry", return_value=mock_registry):
...
def test_list_empty(runner: CliRunner) -> None:
    ...
    with mock.patch("glean.toolkit.cli.get_registry", return_value=empty_registry):
        result = runner.invoke(app, ["list"])
